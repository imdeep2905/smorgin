...
import { FormControl, FormLabel, Input, Button, FormErrorMessage, useToast, Heading, Spinner, Flex} from "@chakra-ui/react"
...
const BinanceNewCredsForm = (props) => {
  ...
  return (
    <Formik
    initialValues={{ api_key: "", secret_key: "" }}
    ...
    {(props) => (
      <Form>
        <Field name="api_key" validate={validateAPIKey}>
          {({ field, form }) => (
            <FormControl colorScheme="teal" isInvalid={form.errors.api_key && form.touched.api_key}>
        ...
        <Field name="secret_key" validate={validateSecretKey}>
          {({ field, form }) => (
            <FormControl colorScheme="teal" isInvalid={form.errors.secret_key && form.touched.secret_key}>
              <FormLabel htmlFor="secret_key">Secret Key</FormLabel>
              <Input {...field} type="password" id="secret_key" placeholder="Your Secret Key" />
...
const BinanceConnectForm = (props) => {
  ...
  if(savedCreds === null) {
    ...
    return (
      savedCreds.api_key == null
      ?
        <BinanceNewCredsForm redirectTo={redirectTo} />
      :
        <BinanceDeleteCredsForm redirectTo={redirectTo} apiKey={savedCreds.api_key} />
    );
  }
}
